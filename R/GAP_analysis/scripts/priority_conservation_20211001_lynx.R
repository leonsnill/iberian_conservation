...
st_write(poly_suit_top15, "results/lynx/lynx_top15_suitability.shp", overwrite=TRUE)  
...
#st_write(poly_suit_top15, "R/new_analysis_05_2021/results_lynx/poly_suit_top15.shp", overwrite=TRUE)  
...
st_write(poly_disp_vague, "results/lynx/poly_disp_vague.shp", overwrite=TRUE)  
##################
...
st_write(network2000_mask, "results/lynx/network_2000_mask.shp", overwrite=TRUE)  
...
network2000_mask <- st_read("results/lynx/network_2000_mask.shp")
network2000_mask_s = st_simplify(network2000_mask, dTolerance = 1000)  # 2000 m
network2000_mask_s <- st_as_sf(network2000_mask_s)
network2000_mask_s = st_transform(network2000_mask_s, crs = crs_project)
...
st_write(diff_PA_dispersal, "results/lynx/dispersal_vague_noPA.shp", overwrite=TRUE)  


# Buffer around Network 2000. Set at 50 Km first. 
...
network2000_mask_buffer <- regional_seas(
  x = network2000_mask_s,
  group = "SITECODE",
  dist = units::set_units(10, km), # buffer distance
  density = units::set_units(0.5, 1/km) # density of points (the higher, the more precise the region attribution)
)
...
st_write(int_buffer_2, "results/lynx/int_buffer_10.shp", overwrite=TRUE)  
...
st_write(poly_disp_restrictive, "results/lynx/poly_disp_restrictive.shp", overwrite=TRUE)  
...
# ----------------------------------------------------------------------------------------------------
# We just delete from the vague scenario the area that it is colonize in the restrictive scenario. The result should
#indicate us where the human has a strong impact on the carnivore colonization
low_colonize <- st_erase(poly_disp_vague, poly_disp_restrictive)
plot(st_geometry(low_colonize))
st_write(low_colonize, "results/lynx/low_colonize.shp", overwrite=TRUE)  
